...
{
    public class CP_IY_Tests : BaseTest
    {
        ...
        public void Test_CP_IY(int offset, byte initialValue, byte valueToAdd, ConditionFlags expectedFlags)
        {
            var rom = AssembleSource($@"
                org 00h
                CP (IY {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
                HALT
            ");
