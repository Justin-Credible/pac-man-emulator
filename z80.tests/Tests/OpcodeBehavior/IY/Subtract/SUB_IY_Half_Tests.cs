...
{
    public class SUB_IY_Half_Tests : BaseTest
    {

        [Fact]
        public void Test_SUB_IYH_ArbitraryExample()
        {
        ...
        [Fact]
        public void Test_SUB_IYL_ArbitraryExample()
        {
            var rom = AssembleSource($@"
                org 00h
                LD A, 0x42
        ...
        public void Test_SUB_IY_Half(bool high, byte initialValue, UInt16 valueToAdd, byte expectedValue, ConditionFlags expectedFlags)
        {
            var rom = AssembleSource($@"
                org 00h
                SUB {(high ? "IYH" : "IYL")}
