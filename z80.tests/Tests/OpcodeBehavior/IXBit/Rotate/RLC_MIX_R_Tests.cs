...
{
    public class RLC_MIX_R_Tests : BaseTest
    {
        ...
        public void Test_RLC_MIX_R_SetsCarryFlagTrue(Register register, int offset)
        {
        ...
        [Theory]
        [MemberData(nameof(GetDataForRegisters))]
        public void Test_RLC_MIX_R_SetsCarryFlagFalse(Register register, int offset)
        {
            var rom = AssembleSource($@"
                org 00h
                RLC (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)}), {register}
        ...
        [InlineData(-33)]
        [InlineData(-62)]
        public void Test_RLC_MIX_SetsCarryFlagTrue(int offset)
        {
            var rom = AssembleSource($@"
                org 00h
        ...
        [InlineData(-33)]
        [InlineData(-62)]
        public void Test_RLC_MIX_SetsCarryFlagFalse(int offset)
        {
            var rom = AssembleSource($@"
                org 00h
                RLC (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
