...
{
    public class SUB_MIX_Tests : BaseTest
    {
        [Fact]
        public void Test_SUB_MIX_ExampleFromManual()
        {
        ...
        public void Test_SUB_MIX(int offset, byte initialValue, byte valueToAdd, byte expectedValue, ConditionFlags expectedFlags)
        {
            var rom = AssembleSource($@"
                org 00h
                SUB (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
                HALT
            ");
