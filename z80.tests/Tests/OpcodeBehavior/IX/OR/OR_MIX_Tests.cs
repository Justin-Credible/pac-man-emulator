...
{
    public class OR_MIX_Tests : BaseTest
    {
        ...
        public void Test_OR_MIX(int offset, byte initialValue, byte valueToOR, byte expectedValue, ConditionFlags expectedFlags)
        {
            var rom = AssembleSource($@"
                org 00h
                OR (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
                HALT
            ");
