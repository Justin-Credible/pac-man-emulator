...
{
    public class INC_MIX_Tests : BaseTest
    {
        ...
        public void Test_INC_MIX_NoFlags(int offset)
        {
            var rom = AssembleSource($@"
                org 00h
                INC (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
                HALT
            ");
        ...
        [Theory]
        [MemberData(nameof(GetData))]
        public void TestINR_MIX_ParityFlag(int offset)
        {
            var rom = AssembleSource($@"
                org 00h
                INC (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
        ...
        [Theory]
        [MemberData(nameof(GetData))]
        public void TestINR_MIX_SignFlag(int offset)
        {
            var rom = AssembleSource($@"
                org 00h
                INC (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
        ...
        [Theory]
        [MemberData(nameof(GetData))]
        public void TestINR_MIX_ZeroButNoCarryFlag(int offset)
        {
            var rom = AssembleSource($@"
                org 00h
                INC (IX {(offset < 0 ? '-' : '+')} {Math.Abs(offset)})
